...
function absolutizeSet($, rootUrl, $content) {
  $('[srcset]', $content).each(function (_, node) {
    ...
    if (urlSet) {
      ...
      var absoluteUrlSet = _toConsumableArray__default['default'](new Set(absoluteCandidates)).join(', ');
...
var GenericContentExtractor = {
  ...
  extract: function extract(_ref, opts) {
    ...
    var _iterator = _createForOfIteratorHelper$2(Reflect.ownKeys(opts).filter(function (k) {
      return opts[k] === true;
    })),
        _step;
